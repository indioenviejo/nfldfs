...
int main(void)
{
  ...
  glp_set_row_name(mip, 2, "WR");
  glp_set_row_bnds(mip, 2, GLP_DB, 0.0, 3.0);
  glp_set_row_name(mip, 3, "TE");
  glp_set_row_bnds(mip, 3, GLP_DB, 0.0, 2.0);
  glp_set_row_name(mip, 4, "RB");
  glp_set_row_bnds(mip, 4, GLP_DB, 0.0, 2.0);
  glp_set_row_name(mip, 5, "Cost");
  glp_set_row_bnds(mip, 5, GLP_DB, 0.0, 46400.0);  
  ...
  double myvariable;
  int i;
  myfile=fopen("/home/raghav/coeff.csv", "r");
  for (i = 0 ; i < numPlayers; i++)
  {
    ...
    int ia[1+796], ja[1+796];
    double ar[1+796];int j;
  ...
  myfile=fopen("/home/raghav/salaries.csv", "r");
  printf("%d\n ",counter);
  printf("-------------------------\n");
  for (j = 1 ; j <= 398; j++)
  {
  ...
  glp_load_matrix(mip, 796, ia, ja, ar);
